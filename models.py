...
def DeeplabV3Plus(channel = 3, nclasses = None):
  ...
  #results = tf.keras.layers.Lambda(lambda x: tf.clip_by_norm(x, 1), name = 'prototype')(results);
  results = tf.keras.layers.Conv2D(nclasses, kernel_size = (1,1), padding = 'same', activation = tf.keras.activations.softmax, use_bias = False, kernel_constraint = tf.keras.constraints.UnitNorm(axis = 2), name = 'full_conv')(results);
  return tf.keras.Model(inputs = inputs, outputs = results);
